...
final class VisitDimensionNormalizer {
    ...
    static Normalized normalize(VisitInfo visitInfo, int maxDimValueLen, int maxUaRawLen, int maxTrackingValueLen) {
        String refererDomain = normalizeRefererDomain(visitInfo == null ? null : visitInfo.referer(), maxDimValueLen);
        String language = normalizePrimaryLanguage(visitInfo == null ? null : visitInfo.acceptLanguage(), 32);

        String uaRaw = truncate(cleanInline(visitInfo == null ? null : visitInfo.userAgent()), maxUaRawLen);
        UaInfo ua = parseUserAgent(uaRaw);

        Tracking t = extractTracking(visitInfo == null ? null : visitInfo.trackingParams(), maxTrackingValueLen);
